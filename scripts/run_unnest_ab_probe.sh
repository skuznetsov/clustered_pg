...
BEGIN
  ...
  v_select_hits := (v_select_iters::bigint * v_probe_size::bigint);
  ...
  FOR v_run IN 1..v_runs LOOP
    FOR v_storage IN SELECT unnest(ARRAY['heap','clustered']) LOOP
      ...
      SELECT array_agg(x)
        INTO v_probe_ids
      FROM generate_series((v_run::bigint - 1) * v_insert_rows + 1,
                           (v_run::bigint - 1) * v_insert_rows + v_probe_size) AS g(x);

      FOR s IN 1..v_warmup_selects LOOP
        EXECUTE 'SELECT count(*) FROM bench_unnest_t t JOIN unnest($1::bigint[]) u(id) ON t.id = u.id'
          INTO v_cnt
          USING v_probe_ids;
        IF v_cnt <> v_probe_size THEN
          RAISE EXCEPTION 'join_unnest warmup mismatch: storage %, run %, got %, expected %', v_storage, v_run, v_cnt, v_probe_size;
        END IF;
      END LOOP;
      ...
      FOR s IN 1..v_select_iters LOOP
        EXECUTE 'SELECT count(*) FROM bench_unnest_t t JOIN unnest($1::bigint[]) u(id) ON t.id = u.id'
          ...
          USING v_probe_ids;
        IF v_cnt <> v_probe_size THEN
          RAISE EXCEPTION 'join_unnest mismatch: storage %, run %, got %, expected %', v_storage, v_run, v_cnt, v_probe_size;
        END IF;
      END LOOP;
      ...
      FOR s IN 1..v_warmup_selects LOOP
        EXECUTE 'SELECT count(*) FROM bench_unnest_t t WHERE t.id = ANY($1::bigint[])'
          ...
          USING v_probe_ids;
        IF v_cnt <> v_probe_size THEN
          RAISE EXCEPTION 'any_array warmup mismatch: storage %, run %, got %, expected %', v_storage, v_run, v_cnt, v_probe_size;
        END IF;
      END LOOP;
      ...
      FOR s IN 1..v_select_iters LOOP
        EXECUTE 'SELECT count(*) FROM bench_unnest_t t WHERE t.id = ANY($1::bigint[])'
          ...
          USING v_probe_ids;
        IF v_cnt <> v_probe_size THEN
          RAISE EXCEPTION 'any_array mismatch: storage %, run %, got %, expected %', v_storage, v_run, v_cnt, v_probe_size;
        END IF;
      END LOOP;
...
echo "unnest_ab_probe: runs=$RUNS batch_size=$BATCH_SIZE batches=$BATCHES select_iters=$SELECT_ITERS probe_size=$PROBE_SIZE status=ok"
